...
class AdminMoneyController extends AdminSet
{
    /**
     * 生成首页
     *
    ...
    public function actionIndex()
    {
        ...
        $pages['tmstart'] = Yii::app()->getRequest()->getParam("tmstart",date('Ym')); //开始月份
        $pages['tmstop'] = Yii::app()->getRequest()->getParam("tmstop",date('Ym')); //结束月份
        $pages['srh_service'] = Yii::app()->getRequest()->getParam("srh_service",""); //按餐厅名称查询
            ...
            if (strtolower($_tmp_pathinfo['extension'])=="csv") {
                ...
                if ($b_mkdir && is_dir($filepathh)) {
                    ...
                    if (move_uploaded_file($_FILES['obj']['tmp_name'], $dest_file_path)) {
                        $msg["msg"] = AppBsMoney::model()->storeCsv($dest_file_path,$type,$month);
                        $msg["code"] = 0;
